...
if (getCode(countryName) === undefined) {
  // if it's undefined it is not a well-formatted country name
  console.log(chalk.red("Please enter a well-formatted country name"));
} else {
  console.log(
    chalk.green(`The code of the country is : `) +
      chalk.bold(`${getCode(countryName)}`)
  );
  if (year === undefined) {
...
async function getData() {
  try {
    ...
    const res = await fetch(
      `https://date.nager.at/api/v3/PublicHolidays/${year}/${getCode(
        countryName
      )}`
    );
